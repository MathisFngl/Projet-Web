<?php
    ...
    function constructionTableau($bdd, $data_amount){
        $ID_Action = 2;
        $requHistoriquePrix = $bdd->prepare("SELECT prix, mois FROM historiqueaction WHERE ID_Action = ? ORDER BY mois DESC LIMIT ?");
        $requHistoriquePrix -> execute(array($ID_Action, $data_amount));
